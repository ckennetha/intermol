...
class SparseAutoencoder(nn.Module):
    def __init__(
        ...
        self.w_enc = nn.Parameter(torch.empty(model_dim, self.hidden_dim))
        self.w_dec = nn.Parameter(torch.empty(self.hidden_dim, model_dim))

        self.b_enc = nn.Parameter(torch.zeros(self.hidden_dim))
        self.b_pre = nn.Parameter(torch.zeros(model_dim))
        ...
        self.register_buffer(
            "stats_last_nonzero", torch.zeros(self.hidden_dim, dtype=torch.long)
        )
...
# dCE
def delta_ce(
    ori_logits: torch.Tensor, recons_logits: torch.Tensor, tokens: torch.Tensor
) -> float:
